...
struct MANGOS_DLL_DECL npc_eris_havenfireAI : public ScriptedAI
{
    ...
    void SummonedMovementInform(Creature* pSummoned, uint32 /*uiMotionType*/, uint32 uiPointId)
    {
        if (uiPointId == 1)
        {
            if (m_uiSaveCounter >= 10 + m_uiCurrentWave) // When saved peasants exceed maximum peasants, something went wrong ^^
                debug_log("SD2: npc_eris_havenfire: Current wave %u was not reset properly in void WaveFinished().", m_uiCurrentWave);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiPhase)
        {
            ...
            debug_log("SD2: npc_eris_havenfire: No phase detected!");
