        ...
        if(DelayTimer < diff)
        {
            switch (pInstance->GetData(TYPE_STAGE))
            {
                ...
                case 5:
                    break;
                case 6:
                    if (pInstance->GetData(TYPE_CRUSADERS_COUNT) == 0 && pInstance->GetData(TYPE_CRUSADERS) == IN_PROGRESS)
                    {
                        pInstance->SetData(TYPE_STAGE, 0);
...
bool GossipHello_npc_toc_announcer(Player* pPlayer, Creature* pCreature)
{
    ...
    if( pPlayer->IsInCombat() || pInstance->IsEncounterInProgress() || pInstance->GetData(TYPE_EVENT)) 
           return true;
...
struct MANGOS_DLL_DECL boss_lich_king_tocAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if(pPortal = m_creature->SummonCreature(NPC_TRIGGER, SpawnLoc[2].x, SpawnLoc[2].y, SpawnLoc[2].z, 5, TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME))
        {
...
struct MANGOS_DLL_DECL npc_tirion_tocAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (!pInstance || pInstance->GetData(TYPE_EVENT_NPC) != NPC_TIRION)
            return;

        UpdateTimer = pInstance->GetData(TYPE_EVENT_TIMER);

        if (IsHero && pInstance->GetData(TYPE_BEASTS) == IN_PROGRESS)
        {
            if (BeastHeroTimer <= diff)
            {
                switch(beastcount)
                {
                case 1:
                    DoScriptText(-1713503, m_creature);
                    pInstance->SetData(TYPE_EVENT, FIGHT2_SUMMON);
                    pInstance->DoUseDoorOrButton(pInstance->GetSingleGameObjectFromStorage(GO_MAIN_GATE_DOOR)->GetObjectGuid());
                    break;
                case 2:
                    DoScriptText(-1713505, m_creature);
                    pInstance->SetData(TYPE_EVENT, FIGHT3_SUMMON);
                    pInstance->DoUseDoorOrButton(pInstance->GetSingleGameObjectFromStorage(GO_MAIN_GATE_DOOR)->GetObjectGuid());
                    break;
                case 3:
                    break;
        ...
        if (UpdateTimer <= diff)
        {
            switch (pInstance->GetData(TYPE_EVENT))
            {
                case FIGHT1_INTRO_1:
                {
                    ...
                    UpdateTimer = 12000;
                    pInstance->SetData(TYPE_EVENT, FIGHT1_INTRO_2);
                ...
                case FIGHT1_INTRO_6:
                {
                    ...
                    UpdateTimer = 10000;
                    pInstance->SetData(TYPE_EVENT, FIGHT1_SUMMON);
                    pInstance->DoUseDoorOrButton(pInstance->GetSingleGameObjectFromStorage(GO_MAIN_GATE_DOOR)->GetObjectGuid());
                    break;
                }
                case FIGHT1_SUMMON:
                {
                    ...
                    beastcount = 1;
                    UpdateTimer = 10000;
                    pInstance->SetData(TYPE_EVENT, FIGHT1_END);
                    pInstance->DoUseDoorOrButton(pInstance->GetSingleGameObjectFromStorage(GO_MAIN_GATE_DOOR)->GetObjectGuid());
                ...
                case FIGHT2_INTRO_1:
                {
                    DoScriptText(-1713503, m_creature);
                    UpdateTimer = 10000;
                    pInstance->SetData(TYPE_EVENT, FIGHT2_INTRO_2);
                    break;
                }
                case FIGHT2_INTRO_2:
                {
                ...
                case FIGHT2_SUMMON:
                {
                    if (pInstance->GetData(TYPE_BEASTS) != DONE)
                    {
                        ...
                        beastcount = 2;
                        UpdateTimer = 10000;
                        pInstance->SetData(TYPE_EVENT, FIGHT2_END);
                ...
                case FIGHT3_INTRO_1:
                {
                    DoScriptText(-1713505, m_creature);
                    UpdateTimer = 15000;
                    pInstance->SetData(TYPE_EVENT, FIGHT3_INTRO_2);
                    break;
                }
                case FIGHT3_INTRO_2:
                {
                ...
                case FIGHT3_SUMMON:
                {
                    ...
                    beastcount = 3;
                    UpdateTimer = 10000;
                    pInstance->SetData(TYPE_EVENT, FIGHT3_END);
                    pInstance->DoUseDoorOrButton(pInstance->GetSingleGameObjectFromStorage(GO_MAIN_GATE_DOOR)->GetObjectGuid());
                    break;
                }
                case 400:
    ...
    //Summoning crusaders
                case 3091:
                {
                    ...
                    if (pInstance->GetData(TYPE_DIFFICULTY) == RAID_DIFFICULTY_25MAN_NORMAL ||  pInstance->GetData(TYPE_DIFFICULTY) == RAID_DIFFICULTY_25MAN_HEROIC)
                    {
                        crusaderscount = 12;
                        switch (urand(0,3))
                        {
    ...
    //Crusaders battle end
                ...
                case 6010:
                {
                    // only on heroic difficulty
                    if (pInstance->GetData(TYPE_DIFFICULTY) == RAID_DIFFICULTY_10MAN_HEROIC  ||  pInstance->GetData(TYPE_DIFFICULTY) == RAID_DIFFICULTY_25MAN_HEROIC)
        ...
        if (UpdateTimer <= diff)
        {
            ...
            {                
                case FIGHT1_INTRO_2:
                    ...
                    UpdateTimer = 2000;
                    pInstance->SetData(TYPE_EVENT, FIGHT1_INTRO_3);
                    break;
                case FIGHT1_INTRO_3:
                    m_creature->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_NONE);
                    UpdateTimer = 3000;
        ...
        if (UpdateTimer <= diff)
        {
            ...
            {                
                case FIGHT1_INTRO_4:
                    ...
                    UpdateTimer = 2000;
                    pInstance->SetData(TYPE_EVENT, FIGHT1_INTRO_5);
                    break;
                case FIGHT1_INTRO_5:
                    m_creature->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_NONE);
                    UpdateTimer = 3000;
...
bool GossipHello_npc_argent_mage(Player* pPlayer, Creature* pCreature)
{
    ScriptedInstance* pInstance;
    pInstance = (ScriptedInstance*)pCreature->GetInstanceData();
    char const* _message;
...
bool GossipSelect_npc_argent_mage(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    ScriptedInstance* pInstance;
    pInstance = (ScriptedInstance*)pCreature->GetInstanceData();
    if (!pInstance)
    ...
    if (pInstance->GetData(TYPE_ANUBARAK) != DONE)
        return false;
    pCreature->CastSpell(pPlayer, SPELL_TELEPORT_DALARAN, false);
...
void AddSC_trial_of_the_crusader()
{
    ...
    NewScript = new Script;
    NewScript->Name = "npc_argent_mage";
    NewScript->pGossipHello = &GossipHello_npc_argent_mage;
    NewScript->pGossipSelect = &GossipSelect_npc_argent_mage;
    NewScript->RegisterSelf();
}
