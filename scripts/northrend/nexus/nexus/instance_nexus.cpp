...
struct MANGOS_DLL_DECL instance_nexus : public ScriptedInstance
{
    ...
    void OnCreatureCreate(Creature* pCreature)
    {
        if (!m_uiFaction)
        {
            if (instance && instance->IsDungeon())
            {
                ...
                if(!PlayerList.isEmpty())
                    m_uiFaction = PlayerList.begin()->getSource()->GetTeam();
            }
        }

        bool NeedMorph = m_uiFaction == ALLIANCE;
        ...
        switch(pCreature->GetEntry())
        {
            ...
            case MOB_ALLY_1:
                if(NeedMorph)
                    pCreature->UpdateEntry(MOB_HORDE_1);
                break;
            case MOB_ALLY_2:
                if(NeedMorph)
                    pCreature->UpdateEntry(MOB_HORDE_2);
                break;
            case MOB_ALLY_3:
                if(NeedMorph)
                    pCreature->UpdateEntry(MOB_HORDE_3);
                break;
            case BOSS_STOUTBEARD_ALLY:
                if(NeedMorph)
                    pCreature->UpdateEntry(BOSS_KOLURG_HORDE);
                break;
            case COMMANDER_ALLY:
                if(NeedMorph)
                    pCreature->UpdateEntry(COMMANDER_HORDE);
                break;
        }
    }
    ...
    void SetData(uint32 uiType, uint32 uiData)
    {
        ...
        if (uiData == SPECIAL && m_auiEncounter[0] == SPECIAL && m_auiEncounter[1] == SPECIAL && m_auiEncounter[2] == SPECIAL)
        {
    ...
    void Load(const char* chrIn)
    {
        ...
        for(uint8 i = 0; i < MAX_ENCOUNTER; ++i)
        {
            ...
            if (m_auiEncounter[i] == SPECIAL)
                m_auiEncounter[i] = (i == 3) ? NOT_STARTED : DONE;
        }
