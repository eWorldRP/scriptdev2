...
SDName: boss_hodir
SD%Complete:
SDComment: Encounter overview:
           Hodir casts regular Icicles all the time (as triggered casts from aura, every 2sec, on 10man there are 2sec. breaks every 2 icicles).
...
struct MANGOS_DLL_DECL mob_npc_flashFreezeAI : public Scripted_NoMovementAI
{
    mob_npc_flashFreezeAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
    {
...
// script for Icicles
struct MANGOS_DLL_DECL mob_icicleAI : public Scripted_NoMovementAI
{
    mob_icicleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
    {
        m_pInstance = (ScriptedInstance*)pCreature->GetInstanceData();
        m_bIsRegularMode = pCreature->GetMap()->IsRegularDifficulty();
        m_uiSpellId = 0;
...
struct MANGOS_DLL_DECL mob_flashFreezeAI : public Scripted_NoMovementAI
{
    mob_flashFreezeAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
    {
        m_pInstance = (ScriptedInstance*)pCreature->GetInstanceData();
