...
struct MANGOS_DLL_DECL boss_xt002AI : public ScriptedAI
{
    ...
    ObjectGuid pLightBombTarGUID;
    ObjectGuid pGravityBombTarGUID;
    ...
    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
        {
            ...
            if(m_bIsHardMode)
            {
                ...
                if(Creature* pHeart = m_pInstance->GetSingleCreatureFromStorage(NPC_HEART))
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiLight_Bomb_Timer < uiDiff && !m_bPhase2)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
            {
                ...
                pLightBombTarGUID = pTarget->GetObjectGuid();
            }
        ...
        if (m_uiGravity_Bomb_Timer < uiDiff && !m_bPhase2)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
            {
                ...
                pGravityBombTarGUID = pTarget->GetObjectGuid();
            }
