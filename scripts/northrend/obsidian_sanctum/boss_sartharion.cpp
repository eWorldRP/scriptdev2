...
enum
{
    ...
    //Sartharion Spells
    SPELL_BERSERK                               = 27680,    // Increases the caster's attack speed by 150% and all damage it deals by 500% for 5 min. Should be 61632, but that doesn't work...
...
struct MANGOS_DLL_DECL boss_sartharionAI : public ScriptedAI
{
            ...
            if (pTemp && pTemp->isAlive() && !pTemp->getVictim())
            {
                ...
                switch(uiEntry)
                {
                    case NPC_TENEBRON:
                        ...
                        pTemp->CastSpell(pTemp, SPELL_POWER_OF_TENEBRON, false);
                        break;
                    case NPC_SHADRON:
                        ...
                        pTemp->CastSpell(pTemp, SPELL_POWER_OF_SHADRON, false);
                        break;
                    case NPC_VESPERON:
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        //berserk - cast if any drake is still alive at Sartharion's 35%
        ...
        // check for alive acolytes or eggs in twilight realm
